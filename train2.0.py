...
# My CNN, now has 5 conv layers with stride 1, padding 1, and uses ReLU
# 5 conv layers for hopefully better accuracy 
class SimpleCNN(nn.Module):
    def __init__(self, num_classes: int, img_size: int, channels=(16, 32, 48, 64, 64)):
        super().__init__()
        c1, c2, c3, c4, c5 = channels
        self.features = nn.Sequential(
            nn.Conv2d(3,  c1, kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(c1, c2, kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(c2, c3, kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(c3, c4, kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(c4, c5, kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True),
        )
        #flat and then linear to class logits
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(c5 * img_size * img_size, num_classes)
        )
...
# Training and writing out results (in a couple forms to a folder called output)
def train(args):
    set_seed(42)
    ...
    channels = (16, 32, 48, 64, 64)  # base

    model = SimpleCNN(num_classes=len(classes), img_size=args.img_size, channels=channels).to(device)
    ...
    with open(out / "metrics.txt", "w", encoding="utf-8") as f:
        f.write("CNN (train2.0) - Test Metrics\n")
...
# for command line
def parse_args():
    p = argparse.ArgumentParser(description="CNN (train2.0)")
    p.add_argument("--data_dir", type=str, default="./ButterflyClassificationDataset")
    p.add_argument("--epochs",   type=int, default=40)     
    p.add_argument("--batch_size", type=int, default=32)
    p.add_argument("--img_size",   type=int, default=64)  
    p.add_argument("--lr",         type=float, default=0.05)
    p.add_argument("--strong_aug", action="store_true", help="use slightly stronger train-time aug")
    return p.parse_args()
