...
# Now with downsampling CNN, batch normalization, and dropout 
class SimpleCNN(nn.Module):
    def __init__(self, num_classes: int, img_size: int, channels=(32, 64, 96, 128, 128), p_drop=0.5):
        ...
        # compute final spatial size (should be img_size // 16)
...
def train(args):
    ...
    model = SimpleCNN(num_classes=len(classes), img_size=args.img_size,
                      channels=channels, p_drop=args.dropout).to(device)
    ...
    with open(out / "metrics.txt", "w", encoding="utf-8") as f:
        f.write("CNN (train2.1) - Test Metrics)\n")
...
# for command line
def parse_args():
    p = argparse.ArgumentParser(description="CNN (train2.1)")
    p.add_argument("--data_dir", type=str, default="./ButterflyClassificationDataset")
    p.add_argument("--epochs",   type=int, default=40)
    p.add_argument("--batch_size", type=int, default=32)
    p.add_argument("--img_size",   type=int, default=64)    # must be divisible by 16
